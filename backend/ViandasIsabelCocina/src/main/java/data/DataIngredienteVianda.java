...
public class DataIngredienteVianda {
        ...
        try {
            try {
                stmt = DbConnector.getInstancia().getConn().prepareStatement(
                        "SELECT iv.codigoIngrediente, iv.idVianda, iv.cantidad, i.nombre AS nombreIngrediente, i.stock, i.unidadMedida, v.nombre AS nombreVianda, v.descripcion, v.precioUnitario, v.tipo, v.activa FROM ingrediente_vianda iv, ingrediente i, vianda v WHERE iv.codigoIngrediente = i.codigo AND iv.idVianda = v.idVianda AND iv.codigoIngrediente = ? AND iv.idVianda = ? ");
    ...
    public IngredienteVianda setIngredienteVianda(IngredienteVianda iv) {
        PreparedStatement stmt = null;
        try {
            try {
                stmt = DbConnector.getInstancia().getConn().prepareStatement(
                        "INSERT INTO ingrediente_vianda (codigoIngrediente, idVianda, cantidad) VALUES (?, ?, ?)");
                stmt.setString(1, iv.getIngrediente().getCodigo());
                stmt.setInt(2, iv.getVianda().getIdVianda());
                stmt.setDouble(3, iv.getCantidad());
                stmt.executeUpdate();
    ...
    public void update(String codigoIngrediente, int idVianda, double cantidad) {
        ...
        String sentencia = "UPDATE ingrediente_vianda SET cantidad = ? WHERE codigoIngrediente = ? AND idVianda = ?";
        try {
            try {
